# function for running DES model in 10K hypothetical donors for main analyses
des_new <- function(N, 
                    donorsex,
                    group,
                    vol,
                    ...
                    streams)
{
  ...
  # applying the DES function to 10K hypothetical donors  
  result <- lapply(1:N, 
                   des.donor, 
                   donorsex, 
                   group,
  ...
  # documenting number of events occurring in 10K hypothetical donors during 18-month follow-up
...
des.donor <- function(i, 
  ...
  ## predicting haemoglobin levels immediately after baseline attendance
  hb.curr <- predict.hb(formula_hb = formula_hb, newdata = covars.i, ranef.hb = ranef.hb, beta.hb = beta.hb)
  hbcurrlist[j] <- hb.curr
  ...
  ## predicting risk of VVR at baseline attendance 
  prob.pulsevvr <- predict(vvr_lr_mod, type = "response", newdata = covars.i, flic = T)
  ...
  repeat {
    ...
    if(event %in% c("donate", "bleed under threshold")){
      ...
      ## line 248 optimised with assistance from ChatGPT
      collectvollist[j] <- pmax((covars.i.new$ebv*covars.i.new$pct_ebv_removed_lagged/100) - 0.025, 0) 

      prob.pulsevvr <- predict(vvr_lr_mod, type = "response", newdata = covars.i.new, flic = T)
      pulsevvr.curr <- rbinom(1, 1, prob.pulsevvr)
    ...
    if(event == "recall"){
      ...
      prob.dropout <- predict(dropout_mod, type = "response", newdata = covars.i.new)
      dropout <- rbinom(1, 1, prob.dropout)
      
      covars.i.new[, agePulse := covars.i$agePulse_new]
    
      nexttime["attend"] <- attend(currenttime, eventlist, timelist, 
                                   covi = covars.i.new, 
                                   fpmod, logcumhaz, curr.recall = curr.recall)
    ...
    if(event == "attend"){
      prob.othdefer <- predict(other_defer_mod, type = "response", newdata = covars.i.new)
    ...
    if(event == "test"){ 
      ...
      hb.curr <- predict.hb(formula_hb = formula_hb, newdata = covars.i.new, ranef.hb = ranef.hb, beta.hb = beta.hb)
      
      if(hb.curr < thresh){ 
        pdefer = inv.logit(hb_defer_mod$coefficients[1])
        d <- runif(1, 0, 1)
        if (d <= pdefer){
  ...
  ## lines 417-418 coded with assistance from ChatGPT
...
# function to simulate attendance times of individual hypothetical donors using flexible parametric survival model
attend.dist <- function(currenttime, eventlist, timelist, covi, fpmod, logcumhaz, curr.recall){
...
# function to draw 400 sets of regression parameters from multivariate normal distributions for probabilistic sensitivity analyses
psa.des <- function(B, 
                    params.b,
                    params.v 
){ 
  ...
  for(b in 1:B){
    pars <- lapply(params, FUN = function(x){
      x[b, ]
    })
    ...
    if(b == 1){
      ...
      psavvr <- data.frame(b, pars$vvr)
      psafpmod <- data.frame(b, pars$fpmod)
      ...
      psavvr <- data.frame(coefname = rownames(psavvr), b, pars$vvr)
      psafpmod <- data.frame(coefname = rownames(psafpmod), b, pars$fpmod)
    }
      ...
      tempvvr <- data.frame(b, pars$vvr)
      tempfpmod <- data.frame(b, pars$fpmod)
      ...
      tempvvr <- data.frame(coefname = rownames(tempvvr), b, pars$vvr)
      tempfpmod <- data.frame(coefname = rownames(tempfpmod), b, pars$fpmod)
      ...
      psavvr <- rbind(psavvr, tempvvr)
      psafpmod <- rbind(psafpmod, tempfpmod)
    }
  }
  
  return(list(params = pars, 
              psahb = psahb,
              psadropout = psadropout,
              psadefer = psadefer,
              psaothdefer = psaothdefer,
              psavvr = psavvr,
              psafpmod = psafpmod)) 
}